...
//        }
        ...
        imageSnapShotFromWords(snapShotString: word, color: color, circular: circular, fontAttributes: fontAttributes)
    }
    ...
    func imageSnapShotFromWords(snapShotString snapShotString: String, color: UIColor?, circular: Bool, fontAttributes: [String : AnyObject]?) {
        ...
        if let attr = fontAttributes {
            ...
            attributes = [
                ...
                NSFontAttributeName : UIFont.systemFontOfSize(CGRectGetWidth(self.bounds) * 0.3)
            ]
        }
